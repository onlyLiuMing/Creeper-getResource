...
chromelauncher
  ...
  .then(chrome => {
    ...
    let TARGET_URL = process.argv[2]; // 传入参数
    let LINKS = []; // 链接列表 待删除
    let TIMER; // 定时器
    ...
    CDP(client => {
      // extract domains
      const {Page, DOM, Network, Runtime} = client;

      Page.addScriptToEvaluateOnNewDocument({scriptSource: 'alert(1)'});
      // 页面框架停止加载js时会调用，
          ...
          .then((args) => {
            ...
            resources.forEach((item, index) => {
              ...
              if (!filename) {
                ...
                if (!filename) {
                  // 如果不是？？开头的特殊url，则为默认的文件名：index(主要是淘宝的域名下会有这些问题，其他页面还没发现有什么特殊的状态)
                  switch (type) {
                    case 'Script':
                      filename = 'index.js';
                      break;
      ...
      function close_client(time) {
        ...
        TIMER = setTimeout(function () {
          client.close(); // 关闭chrome-remote链接
          chrome.kill(); // 关闭chrome
        }, time);
      }
